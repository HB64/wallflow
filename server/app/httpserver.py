...
- POST   /wallpapers/<naam>/seen -> meldt dat deze wallpaper daadwerkelijk als
  achtergrond getoond is (bedoeld voor de Android-app, bij het echt tonen van
  een beeld - niet voor een bestandssysteem-read zoals een Windows-diavoor-
  stelling die toevallig het bestand opent). Bepaalt samen met max_retention_
  days of een wallpaper mag roteren (zie main.py: check_rotation).
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _mark_seen(self, filename):
            ...
            with db.get_connection() as conn:
                found = db.mark_shown(conn, filename)

            if not found:
                self.send_error(404, "Wallpaper niet gevonden in database")
                return
        ...
        def do_POST(self):
            ...
            if path.startswith("/wallpapers/") and path.endswith("/seen"):
                filename = path[len("/wallpapers/"):-len("/seen")].rstrip("/")
                self._mark_seen(filename)
                return
